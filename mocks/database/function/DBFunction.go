...
// SelectPaginateAndFilter provides a mock function with given fields: table, filter, columns, filterMap
func (_m *DBFunction) SelectPaginateAndFilter(table string, filter model.Filter, columns []string, filterMap map[string]string) (*sql.Rows, error) {
	ret := _m.Called(table, filter, columns, filterMap)

	var r0 *sql.Rows
	var r1 error
	if rf, ok := ret.Get(0).(func(string, model.Filter, []string, map[string]string) (*sql.Rows, error)); ok {
		return rf(table, filter, columns, filterMap)
	}
	if rf, ok := ret.Get(0).(func(string, model.Filter, []string, map[string]string) *sql.Rows); ok {
		r0 = rf(table, filter, columns, filterMap)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*sql.Rows)
		}
	...
	if rf, ok := ret.Get(1).(func(string, model.Filter, []string, map[string]string) error); ok {
		r1 = rf(table, filter, columns, filterMap)
	} else {
		r1 = ret.Error(1)
	}
